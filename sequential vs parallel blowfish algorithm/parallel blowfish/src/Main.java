...
public class Main {
    ...
    Main() throws FileNotFoundException {
        ...
        File file=new File("C:\\Users\\Marija Anakieva\\Desktop\\sequential vs parallel blowfish algorithm\\example data\\1000words.txt");
        Scanner scanner=new Scanner(new FileInputStream(file));
        List<String> plainText=new ArrayList<>();
        while(scanner.hasNext()){
            plainText.add(scanner.nextLine());
        }

        int num=100;
        List<Encryptor> threads=new ArrayList<>();
        for(int i=0; i<num; i++){
            List<String> data=plainText.subList(i, (i+1));
            threads.add(new Encryptor(data));
        }
    ...
    class Encryptor extends Thread{
        List<String> data;
        public Encryptor(List<String> data) {
            this.data=data;
        }

        @Override
        public void run() {
            data.forEach(Main.this::encrypt);
        }
    }
